...
export const CreateModal: React.FC<CreateModalProps> = ({ visible, mode, onClose, onCreate }) => {
    ...
    <Modal visible={visible} transparent animationType="fade">
      ...
      >
        <View className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-lg">
          ...
          <View className="mb-4">
            <Text className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Назва</Text>
            <TextInput
              value={name}
          ...
          {mode === 'file' && (
            <View className="mb-6">
              <Text className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Вміст (необов'язково)</Text>
              <TextInput
                value={content}
          ...
          <View className="flex-row justify-end mt-2 gap-3">
            ...
            <TouchableOpacity 
              ...
              disabled={!name.trim()}
            >
              <Text className="text-white font-bold text-sm tracking-tight">Створити</Text>
            </TouchableOpacity>
          </View>
        </View>
      </KeyboardAvoidingView>
